...
| 3.5 | 3 イメージを registry にプッシュするビルドスクリプト作成 | `scripts/build-push.sh` で 3 イメージが registry にプッシュされる | 1.3, 3.1, 3.3, 3.4 | cc:TODO |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 4.1 | Namespace / Project 作成 (`proquip-dev`) | `oc get project proquip-dev` 成功 | 1.2 | cc:TODO |
| 4.2 | Registry pull secret 作成 (内部レジストリの場合は不要) | イメージ pull が動作すること | 4.1, 1.3 | cc:TODO |
| 4.3 | ConfigMap / Secret 作成 (DB 接続情報、Keycloak admin 資格情報) | `oc get configmap proquip-config` / `oc get secret proquip-secrets` 存在 | 4.1, 1.4 | cc:TODO |
...
| 5.3 | CI/CD E2E テスト: feature ブランチ PR → CI パス → main マージ → 自動デプロイ確認 | PR 作成から OpenShift 反映まで自動で完了、Pod が新イメージで起動 | 5.1, Phase 2 | cc:TODO |
| 5.4 | デプロイ手順書更新 | `docs/deployment/rosa-setup.md` を OpenShift Open Environment 向けに更新 | 5.2, 5.3 | cc:TODO |
